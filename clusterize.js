...
;(function(name, definition) {
    ...
    var Clusterize = function(data) {
        ...
        var defaults = {
            ...
            callbacks: {},
            persistence: false,
            persistenceTimeInterval: 500
        }

        // public parameters
        self.options = {};
        var options = ['rows_in_block', 'blocks_in_cluster', 'show_no_data_row', 'no_data_class', 'no_data_text', 'keep_parity', 'tag', 'callbacks', 'persistence', 'persistenceTimeInterval'];
        ...
        if (self.options.persistence) {
            self.persistenceFn = function () {
                ...
                var clusterizeCounter = Number(self.content_elem.style['counter-increment'].replace(/[^0-9]/g, ''));
                
                if (clusterizeCounter > 0) {
                    clusterizeCounter++;
                }
    
                if (currentRows.length > 0) {
                    //do a simple check to see if persistence should be running or not
                    ...
                    if (!shouldPersist) {
                        ...
                        currentRows.forEach(function (tr) {
                            //Update any inputs
                            var inputs = tr.querySelectorAll('input:not([type="checkbox"])');
                            ...
                            //Update textareas
                            var textareas = tr.querySelectorAll('textarea');
                        ...
                        var iterator = clusterizeCounter + (self.options['rows_in_block'] * self.options['blocks_in_cluster']);
        
                        if (iterator > rows.length) {
                            iterator = rows.length;
                        }
        
                        for (var i = clusterizeCounter, _i = 0; i < iterator; i++, _i++) {
                            rows[i] = currentRows[_i].outerHTML;
                        }
                    }
                }
                else {
